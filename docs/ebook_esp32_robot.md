...
## Kompleksowy Przewodnik po Budowie Robota ESP32-CAM

---

**Autor:** Tutorial wygenerowany na podstawie projektu ESP32-CAM Robot Controller

**Wersja:** 1.0

**Wymagania wstępne:** Podstawowa znajomość języka C (zmienne, funkcje, pętle, tablice)
...
## CZĘŚĆ I: FUNDAMENTY
- [Rozdział 1: Zaawansowane Koncepty Języka C](#rozdział-1-zaawansowane-koncepty-języka-c)
- [Rozdział 2: Wprowadzenie do Programowania Mikrokontrolerów](#rozdział-2-wprowadzenie-do-programowania-mikrokontrolerów)
- [Rozdział 3: ESP32 - Architektura i Ekosystem](#rozdział-3-esp32---architektura-i-ekosystem)
- [Rozdział 4: Komponenty Sprzętowe Projektu](#rozdział-4-komponenty-sprzętowe-projektu)

## CZĘŚĆ II: BUDOWA PROJEKTU
- [Rozdział 5: Struktura Projektu i Konfiguracja](#rozdział-5-struktura-projektu-i-konfiguracja)
- [Rozdział 6: Komponent robot_hal - Warstwa Abstrakcji Sprzętu](#rozdział-6-komponent-robot_hal---warstwa-abstrakcji-sprzętu)
- [Rozdział 7: Komponent motor_control - Sterowanie Silnikami](#rozdział-7-komponent-motor_control---sterowanie-silnikami)
- [Rozdział 8: Komponent servo_control - Sterowanie Wieżą](#rozdział-8-komponent-servo_control---sterowanie-wieżą)
- [Rozdział 9: Komponent wifi_manager - Zarządzanie WiFi](#rozdział-9-komponent-wifi_manager---zarządzanie-wifi)
- [Rozdział 10: Komponent camera_stream - Streaming MJPEG](#rozdział-10-komponent-camera_stream---streaming-mjpeg)
- [Rozdział 11: Komponent safety_handler - Bezpieczeństwo](#rozdział-11-komponent-safety_handler---bezpieczeństwo)
- [Rozdział 12: Komponent robot_core - Fasada](#rozdział-12-komponent-robot_core---fasada)

## CZĘŚĆ III: HTTP API
- [Rozdział 13: Komponent http_server - Serwer REST](#rozdział-13-komponent-http_server---serwer-rest)
- [Rozdział 14: API Handlers - Endpointy REST](#rozdział-14-api-handlers---endpointy-rest)

## CZĘŚĆ IV: TESTOWANIE I DEPLOYMENT
- [Rozdział 15: Pisanie Testów z Unity Framework](#rozdział-15-pisanie-testów-z-unity-framework)
- [Rozdział 16: Budowanie, Flashowanie i Debugging](#rozdział-16-budowanie-flashowanie-i-debugging)
- [Rozdział 17: Gotowy Projekt i Dalszy Rozwój](#rozdział-17-gotowy-projekt-i-dalszy-rozwój)

## DODATKI
- [Dodatek A: Pełna Lista Referencji do Dokumentacji](#dodatek-a-pełna-lista-referencji-do-dokumentacji)
- [Dodatek B: Słownik Polsko-Angielski](#dodatek-b-słownik-polsko-angielski)
- [Dodatek C: Diagram Zależności Komponentów](#dodatek-c-diagram-zależności-komponentów)
...
### 1.2.3 Przykład z projektu: motor_control.h

Przyjrzyjmy się rzeczywistej strukturze z naszego projektu. Plik `motor_control.h` definiuje konfigurację sterowania silnikami:
...
| Linia | Kod | Wyjaśnienie |
|-------|-----|-------------|
| 27 | `typedef struct {` | Rozpoczynamy definicję nowego typu strukturalnego |
| 28 | `gpio_num_t in1;` | Pole `in1` typu `gpio_num_t`[^1] - numer pinu GPIO dla kierunku "do przodu" |
| 29 | `gpio_num_t in2;` | Pole `in2` - numer pinu GPIO dla kierunku "do tyłu" |
| 30 | `} motor_pins_t;` | Kończymy strukturę i nadajemy jej nazwę typu `motor_pins_t` |

[^1]: `gpio_num_t` - typ zdefiniowany w ESP-IDF reprezentujący numer pinu GPIO. Zobacz: [ESP-IDF GPIO Driver](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/gpio.html)

### 1.2.4 Struktury zagnieżdżone

Struktury mogą zawierać inne struktury jako pola. Jest to potężna technika organizacji kodu:
...
| Linia | Kod | Wyjaśnienie |
|-------|-----|-------------|
| 35 | `typedef struct {` | Rozpoczęcie definicji struktury konfiguracyjnej |
| 36 | `motor_pins_t left_motor;` | Zagnieżdżona struktura - piny lewego silnika |
| 37 | `motor_pins_t right_motor;` | Zagnieżdżona struktura - piny prawego silnika |
| 38 | `gpio_num_t enable_pin;` | Pin PWM włączający oba silniki |
| 39 | `uint32_t pwm_frequency_hz;` | Częstotliwość PWM w hercach (typ 32-bitowy bez znaku)[^2] |
| 40 | `uint32_t ramp_duration_ms;` | Czas trwania "soft-start" w milisekundach |
| 41 | `uint8_t ramp_steps;` | Liczba kroków rampy (typ 8-bitowy, 0-255) |
| 42 | `} motor_control_config_t;` | Zakończenie i nazwa typu |

[^2]: `uint32_t`, `uint8_t` - typy o gwarantowanym rozmiarze zdefiniowane w `<stdint.h>`. Zobacz: [C Standard Integer Types](https://en.cppreference.com/w/c/types/integer)

### 1.2.5 Inicjalizacja designowana (Designated Initializers)

C99 wprowadził "designated initializers" - możliwość inicjalizacji pól struktury po nazwie. Jest to preferowany sposób w programowaniu embedded, ponieważ:

1. Kod jest bardziej czytelny
2. Kolejność pól nie ma znaczenia
3. Pominięte pola są automatycznie zerowane
...
**Zadanie:** Zaprojektuj strukturę `sensor_config_t` do przechowywania konfiguracji czujnika odległości HC-SR04:
- Pin TRIGGER (wyjście)
- Pin ECHO (wejście)
- Maksymalny zasięg w centymetrach
...
Wskaźnik to zmienna przechowująca adres w pamięci. W programowaniu embedded wskaźniki są niezbędne, ponieważ:

1. Pozwalają na bezpośredni dostęp do rejestrów sprzętowych
2. Umożliwiają przekazywanie dużych struktur bez kopiowania
3. Są podstawą dynamicznej alokacji pamięci
...
Gdy pracujemy ze strukturami, często przekazujemy je przez wskaźnik zamiast przez wartość. Dzięki temu:
...
> **Operator `->` vs `.`:**
> - `struktura.pole` - dostęp do pola gdy mamy zmienną struktury
> - `wskaznik->pole` - dostęp do pola gdy mamy wskaźnik (równoważne z `(*wskaznik).pole`)

### 1.3.3 Wskaźnik const - ochrona danych

Słowo kluczowe `const` przy wskaźniku ma różne znaczenie w zależności od pozycji:
...
Wskaźniki do funkcji pozwalają na "przekazywanie funkcji jako argumentów". Są używane w:
...
W pliku `wifi_manager.c` widzimy praktyczne użycie wskaźników do funkcji:
...
| Element | Wyjaśnienie |
|---------|-------------|
| `esp_event_handler_instance_register`[^3] | Funkcja ESP-IDF rejestrująca callback |
| `&wifi_event_handler` | Adres naszej funkcji obsługi zdarzeń |
| `WIFI_EVENT`[^4] | Kategoria zdarzeń WiFi |
| `ESP_EVENT_ANY_ID` | Obsłuż wszystkie zdarzenia z tej kategorii |

[^3]: `esp_event_handler_instance_register()` - zobacz: [ESP-IDF Event Loop](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_event.html)
[^4]: `WIFI_EVENT` - baza zdarzeń WiFi, zobacz: [ESP-IDF WiFi Events](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/network/esp_wifi.html#events)
...
Preprocesor C to program uruchamiany przed kompilacją właściwą. Przetwarza on dyrektywy rozpoczynające się od `#`:
...
> **Uwaga:** Makra wieloliniowe wymagają znaku `\` na końcu każdej linii oprócz ostatniej.

### 1.4.3 Include Guards - Ochrona przed wielokrotnym włączeniem

Problem: Jeśli ten sam plik nagłówkowy zostanie włączony więcej niż raz, kompilator zgłosi błąd ponownej definicji.

**Rozwiązanie - include guards:**
...
1. Pierwsze włączenie: `MOTOR_CONTROL_H` nie istnieje → preprocesor definiuje go i przetwarza zawartość
2. Kolejne włączenia: `MOTOR_CONTROL_H` już istnieje → preprocesor pomija całą zawartość
...
ESP-IDF używa systemu Kconfig (pochodzącego z jądra Linux) do konfiguracji projektu. Wartości z `menuconfig` są dostępne jako makra `CONFIG_*`:
...
Język C nie ma słów kluczowych `private` ani `public` jak C++ czy Java. Jednak możemy osiągnąć podobny efekt używając `static`.

### 1.5.2 static dla zmiennych globalnych

Zmienna `static` na poziomie pliku jest widoczna **tylko w tym pliku**:
...
| Linia | Kod | Wyjaśnienie |
|-------|-----|-------------|
| 1 | `static struct {` | Anonimowa struktura, widoczna tylko w tym pliku |
...
| 5 | `} s_motor = {0};` | Zmienna `s_motor` zainicjalizowana zerami |

> **Konwencja:** Zmienne statyczne na poziomie pliku często rozpoczynamy od `s_` (np. `s_motor`, `s_wifi`, `s_servo`). Jest to konwencja ESP-IDF i ułatwia rozpoznawanie zmiennych stanu.

### 1.5.3 static dla funkcji

Funkcja `static` jest widoczna tylko w pliku, w którym została zdefiniowana:
...
**Zadanie:** Przeanalizuj poniższy kod i określ, które elementy są publiczne, a które prywatne:
...
| Element | Publiczny/Prywatny | Uzasadnienie |
|---------|-------------------|--------------|
...
| `calibration_offset` | Prywatny | `static` w .c, niewidoczny na zewnątrz |
| `apply_calibration()` | Prywatny | `static` w .c, niewidoczny na zewnątrz |
...
1. **Struktury** - jak grupować dane, używać `typedef`, inicjalizację designowaną
2. **Wskaźniki** - przekazywanie przez referencję, `const`, wskaźniki do funkcji
3. **Preprocesor** - `#define`, include guards, kompilacja warunkowa, Kconfig
4. **static** - enkapsulacja zmiennych i funkcji w C

Te techniki będą wykorzystywane we wszystkich komponentach naszego projektu robota.
...
| Cecha | Procesor PC | Mikrokontroler |
|-------|-------------|----------------|
| Architektura | x86/ARM, wielordzeniowy | ARM/Xtensa, 1-2 rdzenie |
...
- Nieulotna (dane przetrwają wyłączenie zasilania)
- Przechowuje kod programu
- Ograniczona liczba cykli zapisu (~10,000-100,000)

**RAM (pamięć danych):**
- Ulotna (dane tracone po wyłączeniu)
- Przechowuje zmienne, stos, sterta
- Szybki dostęp, ale ograniczona pojemność

**W ESP32-CAM:**
- Flash: 4 MB
- SRAM: 520 KB
- PSRAM: 4 MB (zewnętrzna, do dużych buforów jak klatki kamery)
...
- **Wejście** - odczyt stanu (przycisk, czujnik)
- **Wyjście** - ustawienie stanu (LED, przekaźnik)

### 2.2.2 Stany logiczne

| Stan | Napięcie (3.3V logic) | Wartość w kodzie |
|------|----------------------|------------------|
| LOW  | 0V (GND)             | 0                |
| HIGH | 3.3V (VCC)           | 1                |
...
Aby użyć pinu GPIO, musimy go najpierw skonfigurować. W ESP-IDF używamy struktury `gpio_config_t`[^5]:

[^5]: `gpio_config_t` - zobacz: [ESP-IDF GPIO API](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/gpio.html#_CPPv412gpio_config_t)
...
| Linia | Kod | Wyjaśnienie |
|-------|-----|-------------|
| 13 | `gpio_config_t io_conf = {` | Struktura konfiguracyjna GPIO[^5] |
| 14 | `.pin_bit_mask = (1ULL << pin)` | Maska bitowa wybierająca pin. `1ULL << 12` = bit 12 ustawiony |
| 15 | `.mode = GPIO_MODE_OUTPUT` | Pin będzie wyjściem[^6] |
| 16 | `.pull_up_en = GPIO_PULLUP_DISABLE` | Wyłączenie wewnętrznego rezystora pull-up |
| 17 | `.pull_down_en = GPIO_PULLDOWN_DISABLE` | Wyłączenie rezystora pull-down |
| 18 | `.intr_type = GPIO_INTR_DISABLE` | Bez obsługi przerwań |
| 19 | `gpio_config(&io_conf)` | Zastosowanie konfiguracji[^7] |
| 21 | `gpio_set_level(pin, 0)` | Ustawienie początkowego stanu LOW[^8] |

[^6]: `GPIO_MODE_OUTPUT` - tryb wyjściowy, zobacz: [gpio_mode_t](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/gpio.html#_CPPv411gpio_mode_t)
[^7]: `gpio_config()` - konfiguruje GPIO, zobacz: [gpio_config()](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/gpio.html#_CPPv411gpio_configPK12gpio_config_t)
[^8]: `gpio_set_level()` - ustawia stan pinu, zobacz: [gpio_set_level()](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/gpio.html#_CPPv414gpio_set_level10gpio_num_t8uint32_t)
...
**Zadanie:** Napisz funkcję, która miga wbudowaną diodą LED (GPIO 4 na ESP32-CAM) 5 razy.
...
PWM (Modulacja Szerokości Impulsu) to technika generowania sygnału analogowego za pomocą cyfrowego wyjścia. Zmieniając stosunek czasu włączenia do wyłączenia (duty cycle), możemy kontrolować:
...
|----------|------|-----------------|
| Częstotliwość | Ile razy na sekundę powtarza się cykl | Silniki: 1-20 kHz, Serwa: 50 Hz |
| Duty Cycle | Procent czasu w stanie HIGH | 0-100% |
| Rozdzielczość | Ile poziomów duty cycle | 8-bit (256), 13-bit (8192) |

### 2.3.3 LEDC - PWM w ESP32

ESP32 posiada peryferium LEDC (LED Control), które pomimo nazwy służy do generowania dowolnych sygnałów PWM[^9].

[^9]: LEDC Driver - zobacz: [ESP-IDF LEDC API](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/ledc.html)
...
**Wyjaśnienie kluczowych funkcji ESP-IDF:**

| Funkcja | Opis |
|---------|------|
| `ledc_timer_config()`[^10] | Konfiguruje timer generujący bazową częstotliwość |
| `ledc_channel_config()`[^11] | Łączy timer z pinem GPIO |
| `ledc_set_duty()`[^12] | Ustawia wartość duty cycle |
| `ledc_update_duty()`[^13] | Aktywuje zmianę duty cycle |

[^10]: `ledc_timer_config()` - zobacz: [LEDC Timer Config](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/ledc.html#_CPPv417ledc_timer_configPK18ledc_timer_config_t)
[^11]: `ledc_channel_config()` - zobacz: [LEDC Channel Config](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/ledc.html#_CPPv419ledc_channel_configPK20ledc_channel_config_t)
[^12]: `ledc_set_duty()` - zobacz: [ledc_set_duty](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/ledc.html#_CPPv413ledc_set_duty15ledc_mode_t14ledc_channel_t8uint32_t)
[^13]: `ledc_update_duty()` - zobacz: [ledc_update_duty](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/ledc.html#_CPPv416ledc_update_duty15ledc_mode_t14ledc_channel_t)
...
Serwomechanizmy używają PWM o stałej częstotliwości 50 Hz (okres 20 ms), gdzie pozycja jest określana przez szerokość impulsu:

| Pozycja | Szerokość impulsu | Angle |
|---------|-------------------|-------|
...
RTOS (Real-Time Operating System) to lekki system operacyjny zaprojektowany dla systemów embedded. FreeRTOS[^14] jest najpopularniejszym otwartym RTOS-em i jest domyślnie używany przez ESP-IDF.

[^14]: FreeRTOS - zobacz: [FreeRTOS Documentation](https://www.freertos.org/Documentation.html) oraz [ESP-IDF FreeRTOS](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/freertos.html)

**Główne koncepty:**
- **Task (zadanie)** - niezależny wątek wykonania
- **Scheduler** - przydziela czas procesora zadaniom
- **Priority** - zadania o wyższym priorytecie mają pierwszeństwo
...
**Parametry `xTaskCreate()`[^15]:**

[^15]: `xTaskCreate()` - zobacz: [xTaskCreate API](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/freertos_idf.html#_CPPv411xTaskCreatePF10TaskFunctionPCv14UBaseType_tPv14UBaseType_tPP12TaskHandle_t)

| Parametr | Typ | Opis |
|----------|-----|------|
| `pvTaskCode` | Wskaźnik do funkcji | Funkcja do wykonania w zadaniu |
| `pcName` | String | Nazwa zadania (max 16 znaków) |
| `usStackDepth` | uint16_t | Rozmiar stosu w słowach (×4 na ESP32) |
| `pvParameters` | void* | Argument przekazywany do funkcji |
| `uxPriority` | UBaseType_t | Priorytet (0-24 w ESP-IDF) |
| `pxCreatedTask` | TaskHandle_t* | Handle do późniejszej kontroli |
...
| Funkcja | Opis |
|---------|------|
| `vTaskDelay()`[^16] | Wstrzymuje zadanie na określony czas |
| `ulTaskNotifyTake()`[^17] | Czeka na powiadomienie z innego zadania |
| `xTaskNotifyGive()`[^18] | Wysyła powiadomienie do zadania |
| `xSemaphoreCreateMutex()`[^19] | Tworzy mutex do synchronizacji |
| `xSemaphoreTake()`/`xSemaphoreGive()` | Blokuje/zwalnia mutex |

[^16]: `vTaskDelay()` - zobacz: [vTaskDelay](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/freertos_idf.html#_CPPv410vTaskDelay10TickType_t)
[^17]: `ulTaskNotifyTake()` - zobacz: [Task Notifications](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/freertos_idf.html#task-notification-api)
[^18]: `xTaskNotifyGive()` - zobacz: [xTaskNotifyGive](https://www.freertos.org/xTaskNotifyGive.html)
[^19]: `xSemaphoreCreateMutex()` - zobacz: [Semaphore API](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/freertos_idf.html#semaphore-api)

### 2.4.5 Makro pdMS_TO_TICKS

FreeRTOS operuje na "tickach" - jednostkach czasu schedulera. Makro `pdMS_TO_TICKS()` konwertuje milisekundy na ticki:
...
ESP-IDF dostarcza wygodne API do timerów programowych[^20]. Istnieją dwa rodzaje:

[^20]: ESP Timer - zobacz: [ESP-IDF Timer API](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_timer.html)

- **Periodic** - wywoływany cyklicznie co określony czas
- **One-shot** - wywoływany jednokrotnie po upływie czasu
...
| Funkcja | Opis |
|---------|------|
| `esp_timer_create()`[^21] | Tworzy timer |
| `esp_timer_start_once()`[^22] | Uruchamia timer jednorazowy |
| `esp_timer_start_periodic()`[^23] | Uruchamia timer cykliczny |
| `esp_timer_stop()`[^24] | Zatrzymuje timer |
| `esp_timer_delete()`[^25] | Usuwa timer i zwalnia zasoby |

[^21]: `esp_timer_create()` - zobacz: [esp_timer_create](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_timer.html#_CPPv416esp_timer_createPK22esp_timer_create_args_tP18esp_timer_handle_t)
[^22]: `esp_timer_start_once()` - zobacz: [esp_timer_start_once](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_timer.html#_CPPv420esp_timer_start_once18esp_timer_handle_t8uint64_t)
[^23]: `esp_timer_start_periodic()` - zobacz: [esp_timer_start_periodic](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_timer.html#_CPPv424esp_timer_start_periodic18esp_timer_handle_t8uint64_t)
[^24]: `esp_timer_stop()` - zobacz: [esp_timer_stop](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_timer.html#_CPPv414esp_timer_stop18esp_timer_handle_t)
[^25]: `esp_timer_delete()` - zobacz: [esp_timer_delete](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_timer.html#_CPPv416esp_timer_delete18esp_timer_handle_t)

---

## 2.6 Ćwiczenie 2.2: Zadanie mrugające LED

**Zadanie:** Utwórz zadanie FreeRTOS, które mruga LED co 250 ms, jednocześnie pozwalając głównemu programowi wykonywać inne operacje.
...
1. **GPIO** - konfiguracja pinów jako wejścia/wyjścia
2. **PWM** - generowanie sygnałów o zmiennym wypełnieniu (LEDC)
3. **FreeRTOS** - tworzenie zadań, opóźnienia, powiadomienia
4. **Timery** - periodic i one-shot timery
...
| Cecha | ESP32 | Arduino Uno | STM32F4 | Raspberry Pi Pico |
|-------|-------|-------------|---------|-------------------|
| **Rdzeń** | Dual Xtensa 240MHz | AVR 16MHz | ARM Cortex-M4 168MHz | Dual ARM Cortex-M0+ 133MHz |
| **RAM** | 520 KB | 2 KB | 192 KB | 264 KB |
| **Flash** | 4-16 MB | 32 KB | 512 KB-1 MB | 2 MB |
| **WiFi** | Wbudowane | Brak | Brak | Pico W: tak |
| **Bluetooth** | Wbudowane | Brak | Brak | Brak |
| **Cena** | ~$3-5 | ~$3-5 | ~$5-15 | ~$4-6 |
| **IDE** | ESP-IDF/Arduino | Arduino IDE | STM32CubeIDE | SDK/Arduino |
| **Dla kogo** | IoT, robotyka | Początkujący | Zaawansowani | Hobby/edukacja |

### 3.1.2 Zalety ESP32 dla projektu robota

1. **Wbudowane WiFi** - sterowanie przez HTTP bez dodatkowych modułów
2. **Wystarczająca moc** - dwa rdzenie pozwalają na streaming wideo i sterowanie
3. **PSRAM** - ESP32-CAM ma 4 MB zewnętrznej RAM dla buforów kamery
4. **Bogate peryferia** - PWM, ADC, I2C, SPI w jednym układzie
5. **Niska cena** - ESP32-CAM kosztuje ~$5-10
6. **Duża społeczność** - łatwo znaleźć pomoc i przykłady
...
| GPIO | Funkcja | Dostępność dla projektu |
|------|---------|------------------------|
...
> **Kompromis:** Rezygnujemy z karty MicroSD, aby uwolnić piny dla sterowania silnikami.
...
ESP-IDF (Espressif IoT Development Framework) to oficjalny framework do programowania ESP32[^26]. Oferuje:

[^26]: ESP-IDF - zobacz: [ESP-IDF Programming Guide](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/)
...
**Główny CMakeLists.txt projektu:**
...
**CMakeLists.txt komponentu (np. motor_control):**
...
Kconfig pozwala na konfigurację projektu przez menu tekstowe (`idf.py menuconfig`):
...
1. Pobierz [ESP-IDF Tools Installer](https://dl.espressif.com/dl/esp-idf/)
2. Uruchom instalator i wybierz wersję ESP-IDF v5.2
3. Używaj "ESP-IDF Command Prompt" do budowania projektów
...
1. **Dlaczego ESP32** - porównanie z alternatywami
2. **Architektura ESP32** - dual-core, peryferia, pamięć
3. **ESP32-CAM** - specyfikacja modułu, ograniczenia GPIO
4. **ESP-IDF** - struktura projektu, CMake, Kconfig
5. **Instalacja** - konfiguracja środowiska deweloperskiego
...
- ✅ Zawsze aktywuj środowisko przed budowaniem: `. ~/esp/esp-idf/export.sh`

---

*(Kontynuacja w kolejnych rozdziałach...)*
...
| LEDC (PWM) | https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/ledc.html |
...
| ESP Event | https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_event.html |
...
| Error Handling | https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_err.html |
...
| Polski | English | Opis |
|--------|---------|------|
| Mikrokontroler | Microcontroller | Układ scalony z procesorem, pamięcią i peryferiami |
| Mostek H | H-Bridge | Układ do sterowania kierunkiem silnika DC |
| Wypełnienie | Duty Cycle | Procent czasu w stanie wysokim sygnału PWM |
| Szerokość impulsu | Pulse Width | Czas trwania impulsu w sygnale PWM |
| Strumień | Stream | Ciągły przepływ danych (np. MJPEG) |
| Strażnik | Watchdog | Timer resetujący system przy zawieszeniu |
| Punkt dostępowy | Access Point (AP) | Urządzenie tworzące sieć WiFi |
| Sterownik | Driver | Oprogramowanie kontrolujące sprzęt |
| Peryferium | Peripheral | Wbudowany moduł sprzętowy (GPIO, SPI...) |
| Przerwanie | Interrupt | Sygnał przerywający normalne wykonanie programu |
...
| Callback | Callback | Funkcja wywoływana w odpowiedzi na zdarzenie |
| Inicjalizacja | Initialization | Konfiguracja początkowa modułu |
| Deserializacja | Deserialization | Konwersja danych (np. JSON) na strukturę |
...
Mostek H to układ elektroniczny pozwalający na sterowanie kierunkiem prądu płynącego przez obciążenie (np. silnik DC). Nazwa pochodzi od kształtu schematu przypominającego literę H:
...
**Działanie:**
- Q1+Q4 ON, Q2+Q3 OFF → Prąd płynie od lewej do prawej → Obrót w prawo
- Q2+Q3 ON, Q1+Q4 OFF → Prąd płynie od prawej do lewej → Obrót w lewo
- Wszystkie OFF → Silnik wolny (coast)
- Q1+Q3 lub Q2+Q4 ON → Hamowanie (brake)

### 4.2.2 Tablica prawdy DRV8833

DRV8833 to podwójny mostek H firmy Texas Instruments, sterowany sygnałami IN1/IN2 dla każdego kanału:
...
Problem: Nagłe włączenie silnika powoduje duży prąd rozruchowy, który może:
- Zakłócić zasilanie ESP32
- Uszkodzić sterownik
- Powodować szarpanie mechaniczne
...
Serwomechanizm to silnik z wbudowanym sterownikiem pozycji. Pozycja jest kontrolowana przez szerokość impulsu PWM:
...
**Przykład obliczeń:**
- Dla kąta 90°: `pulse = 500 + (90/180) × 1900 = 500 + 950 = 1450 µs`
- Dla kąta 45°: `pulse = 500 + (45/180) × 1900 = 500 + 475 = 975 µs`
...
**Zadanie:** Oblicz szerokość impulsu PWM potrzebną do ustawienia serwa na kąt 135°, jeśli:
- Minimum: 500 µs = 0°
- Maksimum: 2400 µs = 180°
...
Nasz projekt używa architektury warstwowej, gdzie każda warstwa zależy tylko od warstw niższych:
...
- **Separacja odpowiedzialności** - każdy moduł robi jedną rzecz
- **Testowalność** - możemy mockować warstwy niższe
- **Przenośność** - zmiana HAL pozwala na port na inną platformę
- **Czytelność** - łatwo zrozumieć zależności
...
**NVS (Non-Volatile Storage)**[^27] to system przechowywania danych klucz-wartość w pamięci flash. Jest używany przez WiFi do zapisywania kalibracji i przez inne komponenty.

[^27]: NVS Flash - zobacz: [ESP-IDF NVS API](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/storage/nvs_flash.html)
...
**SPIFFS (SPI Flash File System)**[^28] to prosty system plików dla pamięci flash. Używamy go do przechowywania plików statycznych (HTML, CSS, JS) dla interfejsu webowego.

[^28]: SPIFFS - zobacz: [ESP-IDF SPIFFS API](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/storage/spiffs.html)

### 5.2.2 Funkcja app_main()
...
1. **NVS** - wymagany przez WiFi
2. **SPIFFS** - potrzebny przed startem HTTP (pliki statyczne)
3. **WiFi** - potrzebny przed HTTP (sieć)
4. **Sprzęt** - silniki, serwo, kamera
5. **Robot core** - logika biznesowa
6. **Safety** - watchdog musi być ostatni
7. **HTTP server** - uruchamiamy gdy wszystko gotowe
...
| Partycja | Typ | Offset | Rozmiar | Opis |
|----------|-----|--------|---------|------|
| nvs | data/nvs | 0x9000 | 24 KB | Non-Volatile Storage |
| phy_init | data/phy | 0xf000 | 4 KB | Dane kalibracji WiFi |
| factory | app | 0x10000 | 3 MB | Nasza aplikacja |
| spiffs | data/spiffs | 0x310000 | 960 KB | System plików (web UI) |
...
**HAL (Hardware Abstraction Layer)** izoluje kod od konkretnego sprzętu. Dzięki temu:

1. **Testowalność** - możemy podmienić HAL na mock w testach
2. **Przenośność** - zmiana platformy = zmiana HAL, reszta bez zmian
3. **Czytelność** - ukrywamy szczegóły ESP-IDF

## 6.2 hal_types.h - Definicje typów
...
| Linia | Kod | Wyjaśnienie |
|-------|-----|-------------|
| 6-7 | `#ifndef HAL_TYPES_H` / `#define HAL_TYPES_H` | Include guard |
| 9-10 | `#include <stdbool.h>` / `<stdint.h>` | Typy bool i uint8_t |
| 12-14 | `#ifdef __cplusplus` / `extern "C" {` | Kompatybilność z C++ |
| 19 | `typedef uint8_t hal_pwm_channel_t` | Typ reprezentujący kanał PWM |
| 24 | `#define HAL_PWM_CHANNEL_INVALID 0xFF` | Wartość specjalna = brak kanału |
| 29 | `#define HAL_PWM_MAX_CHANNELS 8` | ESP32 ma 8 kanałów LEDC |
| 34 | `#define HAL_PWM_DUTY_MAX 100` | Duty cycle wyrażony w procentach |
| 39 | `#define HAL_SERVO_PWM_FREQ 50` | Standardowa częstotliwość serwa |

## 6.3 hal_gpio.h - Interfejs GPIO
...
## 6.4 hal_gpio.c - Implementacja GPIO
...
## 6.5 hal_pwm.c - Implementacja PWM
...
Komponent `motor_control` odpowiada za sterowanie dwoma silnikami DC poprzez sterownik DRV8833. Jest to jeden z najważniejszych komponentów projektu - od niego zależy mobilność robota.

**Główne funkcje komponentu:**
- Sterowanie kierunkiem silników (przód, tył, obrót)
- Soft-start - płynne narastanie prędkości
- Zatrzymanie awaryjne

**Zależności:**
- `robot_hal` - abstrakcja GPIO i PWM
- `robot_types.h` - definicje enum kierunków

## 7.2 Plik nagłówkowy motor_control.h
...
### 7.2.1 Wyjaśnienie struktury motor_pins_t

Struktura `motor_pins_t` reprezentuje dwa piny GPIO sterujące jednym silnikiem przez mostek H:
...
## 7.3 Implementacja motor_control.c
...
> **Dokumentacja ESP-IDF:**
> - [esp_err.h - Error Codes](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_err.html)
...
Gdy silnik DC startuje od razu z pełną mocą, występują następujące problemy:
1. **Szarpnięcie mechaniczne** - robot "skacze"
2. **Spike prądowy** - może uszkodzić zasilanie
3. **Hałas** - głośny start silników

Soft-start rozwiązuje te problemy przez stopniowe zwiększanie PWM od 0% do 100%.

### 7.4.2 Plik nagłówkowy pwm_ramper.h
...
### 7.4.3 Implementacja pwm_ramper.c
...
> **Dokumentacja FreeRTOS:**
> - [Task Notifications](https://www.freertos.org/Documentation/02-Kernel/02-Kernel-features/03-Direct-to-task-notifications/00-Direct-to-task-notifications)
> - [Semaphores](https://www.freertos.org/Documentation/02-Kernel/02-Kernel-features/02-Queues-mutexes-and-semaphores/02-Mutexes-and-binary-semaphores)
...
**Zadanie:** Zmodyfikuj `motor_control` aby obsługiwał parametr prędkości (0-100%).

**Wskazówka:** Dodaj parametr `speed_percent` do funkcji ruchu i przekaż go do `pwm_ramper_config_t.max_duty_percent`.
...
**Zadanie:** Dodaj funkcję `motor_brake_all()` która używa trybu BRAKE zamiast COAST.
...
Komponent `servo_control` steruje serwomechanizmem SG90, który obraca wieżyczkę z kamerą. Serwomechanizmy są sterowane sygnałem PWM o stałej częstotliwości 50Hz, gdzie szerokość impulsu określa pozycję kątową.

**Główne funkcje:**
- Konwersja kąta na szerokość impulsu PWM
- Płynny ruch z interpolacją
- Funkcje step left/right dla kroków po 10°

## 8.2 Sterowanie serwomechanizmem - teoria

### 8.2.1 Sygnał PWM dla serwa

Serwomechanizmy RC używają sygnału PWM o częstotliwości **50Hz** (okres 20ms):

| Szerokość impulsu | Kąt |
|-------------------|-----|
| 500µs | 0° |
| 1000µs | 45° |
| 1500µs | 90° (środek) |
...
Przykład dla SG90:
- `min_pulse = 500µs`, `max_pulse = 2400µs`
...
## 8.3 Plik nagłówkowy servo_control.h
...
## 8.4 Implementacja servo_control.c
...
### Ćwiczenie 8.1: Sweep
**Zadanie:** Napisz funkcję `servo_sweep()` która płynnie obraca serwo od min do max i z powrotem.
...
Komponent `wifi_manager` konfiguruje ESP32 jako Access Point (punkt dostępowy). Robot tworzy własną sieć WiFi, do której można połączyć telefon lub laptop, aby sterować robotem.

**Kluczowe cechy:**
- Tryb AP (Access Point) - robot jest "routerem"
- Statyczne IP: 10.42.0.1
- DHCP dla klientów
...
|------|------|--------------|
| **STA** (Station) | Łączy się do istniejącej sieci | Robot w sieci domowej |
| **AP** (Access Point) | Tworzy własną sieć | Robot autonomiczny |
| **AP+STA** | Oba tryby | Most WiFi |

W naszym projekcie używamy **trybu AP** - robot jest niezależny od infrastruktury.

## 9.3 Plik nagłówkowy wifi_manager.h
...
## 9.4 Implementacja wifi_manager.c
...
> - [WiFi Events](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/network/esp_wifi.html#_CPPv418wifi_event_ap_probe_req_rx_t)
...
> - [WiFi Configuration](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-guides/wifi.html)
...
**Zadanie:** Zmodyfikuj `wifi_manager_start_ap()` aby obsługiwał ukryte sieci (SSID nie jest rozgłaszany).

**Wskazówka:** Użyj pola `wifi_config.ap.ssid_hidden`.
...
Komponent `camera_stream` obsługuje kamerę OV2640 wbudowaną w ESP32-CAM i udostępnia strumień MJPEG przez HTTP. MJPEG (Motion JPEG) to format wideo, w którym każda klatka jest osobnym obrazem JPEG.

**Główne funkcje:**
- Inicjalizacja kamery OV2640
- Streaming MJPEG przez endpoint `/stream`
- Konfiguracja jakości i rozdzielczości
...
## 10.3 Plik nagłówkowy camera_stream.h
...
## 10.4 Implementacja camera_stream.c
...
Bezpieczeństwo jest krytyczne w robotyce. Komponent `safety_handler` implementuje mechanizmy zabezpieczające:

1. **Watchdog Timer** - resetuje system jeśli główna pętla się zawiesi
2. **Auto-stop Timer** - automatycznie zatrzymuje silniki po określonym czasie
3. **Walidacja duration** - ogranicza maksymalny czas ruchu

## 11.2 Plik nagłówkowy safety_handler.h
...
## 11.3 Implementacja safety_handler.c
...
Komponent `robot_core` implementuje wzorzec projektowy **Facade** (Fasada). Zamiast bezpośredniego wywoływania `motor_control`, `servo_control` i `safety_handler`, warstwy wyższe (HTTP API) używają prostego interfejsu `robot_move()`, `robot_turret()`, `robot_stop()`.

**Korzyści:**
- Uproszczenie API
- Centralna logika walidacji
- Łatwa wymiana implementacji (mock mode)
- Jeden punkt odpowiedzialności

## 12.2 Plik robot_types.h - Współdzielone typy
...
**Zadanie:** Dodaj funkcję `robot_move_turbo()` która pomija soft-start i od razu ustawia 100% PWM.
...
Komponent `http_server` uruchamia serwer HTTP na ESP32, który obsługuje REST API i serwuje pliki statyczne (Web UI). ESP-IDF dostarcza lekką bibliotekę `esp_http_server` idealną dla embedded.

**Główne funkcje:**
- Uruchomienie serwera HTTP na porcie 4567
- Rejestracja endpointów API
- Serwowanie plików z SPIFFS
...
## 13.3 Plik nagłówkowy http_server.h
...
## 13.4 Implementacja http_server.c
...
## 13.5 Helpery API - api_helpers.c
...
Plik `api_handlers.c` zawiera implementacje wszystkich endpointów REST API. Każdy handler:
1. Odczytuje body żądania (dla POST)
2. Parsuje JSON używając biblioteki cJSON
3. Wywołuje odpowiednią funkcję fasady `robot_core`
4. Zwraca odpowiedź JSON

## 14.2 Biblioteka cJSON

ESP-IDF zawiera wbudowaną bibliotekę [cJSON](https://github.com/DaveGamble/cJSON) do parsowania i generowania JSON:
...
## 14.3 Implementacja api_handlers.c
...
[Unity](http://www.throwtheswitch.org/unity) to lekki framework do testów jednostkowych w C. ESP-IDF zawiera go jako część komponentu `unity`.

**Dlaczego testy?**
- Wykrywanie błędów wcześnie
- Dokumentacja przez przykład
...
## 15.4 Testy robot_core - test_robot.c
...
## 15.5 Testy motor_control - test_motor_control.c
...
| `TEST_ASSERT_EQUAL_FLOAT(exp, act, eps)` | Równość float z tolerancją |
...
**Zadanie:** Napisz test sprawdzający że `safety_validate_duration()` poprawnie ogranicza czas.
...
ESP-IDF używa CMake jako systemu budowania. Każdy komponent ma swój `CMakeLists.txt`:
...
| Partycja | Typ | Rozmiar | Opis |
|----------|-----|---------|------|
| nvs | data | 24KB | Non-Volatile Storage |
| phy_init | data | 4KB | Kalibracja PHY WiFi |
...
| "SPIFFS mount failed" | Uruchom `idf.py build` (generuje spiffs.bin) |
...
| Funkcja | Komponent | Technologia |
|---------|-----------|-------------|
| Sterowanie silnikami | motor_control | DRV8833 + PWM |
| Sterowanie serwem | servo_control | PWM 50Hz |
| WiFi Access Point | wifi_manager | ESP WiFi |
| Streaming kamery | camera_stream | MJPEG + OV2640 |
| REST API | http_server | ESP HTTP Server |
| Bezpieczeństwo | safety_handler | ESP Timer |
| Fasada | robot_core | Wzorzec Facade |
...
1. **Praktyka** - Zbuduj więcej projektów
2. **Dokumentacja** - Czytaj ESP-IDF docs
3. **Społeczność** - Dołącz do forum ESP32
4. **Certyfikaty** - Rozważ kursy Embedded Systems
5. **Portfolio** - Publikuj projekty na GitHub

## 17.5 Podziękowania

Gratulacje! Ukończyłeś kompleksowy kurs budowy robota ESP32-CAM. Masz teraz solidne podstawy do kariery jako Embedded Developer.
...
| SPIFFS | https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/storage/spiffs.html |
| Event Loop | https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_event.html |
| Error Codes | https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/esp_err.html |
| Logging | https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/log.html |
...
| Wejście/Wyjście | Input/Output (I/O) | Kierunek przepływu sygnału |
...
*Dziękujemy za przeczytanie tego e-booka. Mamy nadzieję, że pomógł Ci zrozumieć świat programowania embedded i zbudować Twojego pierwszego robota!*

*Jeśli masz pytania lub sugestie, otwórz Issue na GitHubie projektu.*
